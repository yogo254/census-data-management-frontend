...
import { fileServerAddress } from "../../config/Config";
...
const PersonalDetails = ({
  ...
  const uploadFile = formData => {
    axios
      .post(
        `${fileServerAddress}/api/files/upload/${person.id}`,
        formData,
        config
      )
      .then(res => {
        personUpdate({ avatar: res.data.uri });
      });
  };
      ...
      <div className="col s4 offset-s4">
        ...
        <div className="file-field input-field">
          <div className="btn white blue-text">
            <span>{person.avatar ? "Change Image" : "Upload Image"}</span>
            <input type="file" name="file" onChange={e => updateFormData(e)} />
          </div>
          ...
          <div className="row">
            ...
            <div className="input-field">
              ...
              <input
                ...
                id="dob"
                name="dob"
                defaultValue={moment(person.dob).format("DD-MM-YYYY")}
                required
              />
            </div>
            <div className="input-field">
